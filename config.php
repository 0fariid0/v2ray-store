...
function v2raystore_getAgentServerDeliveryKeys($agentId, $offset = 0){
    ...
    if($stmt){
        ...
        $stmt->close();
        $pager = [];
        if($offset > 0) $pager[] = ['text'=>'« صفحه قبلی', 'callback_data'=>'agSrvPanel_' . $agentId . '_' . max(0, $offset - $limit)];
        if(isset($count) && $count >= $limit) $pager[] = ['text'=>'صفحه بعدی »', 'callback_data'=>'agSrvPanel_' . $agentId . '_' . ($offset + $limit)];
        if($pager) $keys[] = $pager;
    }
...
function getAgentDiscounts($agentId){
    ...
    $keys[] = [
        ['text'=>'🔗 ارسال و دسترسی هر سرور', 'callback_data'=>'agSrvPanel_' . $agentId . '_0']
    ];
    $limits = v2raystore_agentLimitsNormalize($discounts['limits'] ?? []);
    $keys[] = [
