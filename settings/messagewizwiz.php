...
$lockFile = sys_get_temp_dir() . '/wizwiz_broadcast_queue_' . md5(__DIR__) . '.lock';
...
function wizwiz_broadcast_api_error($response){
...
function wizwiz_broadcast_retry_after($response){
    ...
    $desc = wizwiz_broadcast_api_error($response);
    if(preg_match('/retry after\s+(\d+)/i', $desc, $m)) return intval($m[1]);
    return 0;
}

function wizwiz_broadcast_is_blocked_error($response){
    $desc = strtolower(wizwiz_broadcast_api_error($response));
...
function wizwiz_broadcast_send_to_user($info, $targetUserId, $keys){
    $type = $info['type'] ?? 'text';
...
function wizwiz_broadcast_admin_message($text){
    global $admin;
...
while($user = $usersList->fetch_assoc()){
    ...
    $result = wizwiz_broadcast_send_to_user($info, $targetUserId, $keys);
    ...
    if(is_object($result) && isset($result->ok) && $result->ok){
        ...
        $retryAfter = wizwiz_broadcast_retry_after($result);
        if($retryAfter > 0){
            $throttled = true;
        ...
        if(wizwiz_broadcast_is_blocked_error($result)) $blockedCount++;
        else $failedCount++;
...
if($throttled){
    ...
    wizwiz_broadcast_admin_message("⏸ تلگرام محدودیت موقت ارسال داد.\n\nصف متوقف نشد و حدود $waitSec ثانیه دیگر ادامه می‌دهد.\n☑️ پردازش‌شده: $offset\n📨 موفق: $sentCount\n⛔️ ناموفق: $failedCount");
    exit;
}
...
if(($updatedAt - $lastReportAt) >= $progressInterval){
    ...
    $msgTitle = ($type == 'forwardall') ? 'فوروارد همگانی' : 'ارسال پیام همگانی';
    wizwiz_broadcast_admin_message("📊 گزارش پیشرفت $msgTitle\n\n🎯 گروه مخاطب: $targetTitle\n☑️ پردازش‌شده: $offset\n📨 موفق: $sentCount\n⛔️ ناموفق: $failedCount\n🚫 بلاک/غیرفعال: $blockedCount\n📣 باقی‌مانده تقریبی: $left");
