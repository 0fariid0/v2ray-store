...
install_packages() {
    ...
    # Install web/PHP tools separately so a MySQL package name mismatch does not
    # cancel the whole dependency installation on Debian/Ubuntu variants.
    apt-get install -y apache2 php libapache2-mod-php php-mbstring php-zip php-gd php-json php-curl php-soap php-ssh2 git wget curl unzip openssl ca-certificates certbot python3-certbot-apache >/dev/null 2>&1 || true
